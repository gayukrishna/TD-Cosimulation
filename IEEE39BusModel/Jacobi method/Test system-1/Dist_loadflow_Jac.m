function [S_phckt24sub,P_phckt24sub,Q_phckt24sub,V_0sub,V_1sub,V_2sub]=Dist_loadflow_Jac(T1,T3,V_asub,V_bsub,V_csub,V_aangsub,V_bangsub,V_cangsub)
...
fid=fopen('C:\Users\user\Dropbox\personal\MATLABCODES\cosim_codes\Codes\bigger_model\Jacobi method\Test system-1\ckt24\Interfacevoltage.txt','w');
...
DSSText.Command='Compile (C:\Users\user\Dropbox\personal\MATLABCODES\cosim_codes\Codes\bigger_model\Jacobi method\Test system-1\ckt24\master_ckt24_cosim.dss)';
%DSSText.Command = ['Redirect Test_cases\scenario_',int2str(scenario),'_combined','\PV_combined',int2str(cust_per),'per.txt'];
%DSSText.Command='New	PVSystem.PV_1.1	phases=1	bus1=SubXfmr_LSB.1	kV=19.9   kVA=10000	irradiance=1	Pmpp=10000	%cutin=0.1	%cutout=0.1';		
...
% V_asub= 1.05;
% V_bsub= 1.05*(cos(-120/180)*pi + 1i*sin(-120/180)*pi );
% V_csub= 1.05*(cos(120/180)*pi + 1i*sin(120/180)*pi );
