function [S_a,S_b,S_c,P_0seq,P_1seq,P_2seq,Q_0seq,Q_1seq,Q_2seq,VaT,VbT,VcT,V0T,V1T,V2T]=Trans_loadflow_multifeeder_Jac(Sl_aupdate,Sl_bupdate,Sl_cupdate)
%% data input
...
% phase powers at transmission connected load buses
for k=1:Nload
...
% sequence powers at transmission connected load buses
for k=1:Nload
P_0seq(k)=abs(Pload_0sp(Loads(k),1));
P_1seq(k)=abs(Pload_1sp(Loads(k),1));
P_2seq(k)=abs(Pload_2sp(Loads(k),1));
Q_0seq(k)=abs(Qload_0sp(Loads(k),1));
Q_1seq(k)=abs(Qload_1sp(Loads(k),1));
Q_2seq(k)=abs(Qload_2sp(Loads(k),1));
end

P_Seq=[P_0seq; P_1seq; P_2seq];
Q_Seq=[Q_0seq; Q_1seq; P_2seq];

% voltages and angles at transmission connected load buses
for k=1:Nload
    VaT(k)=Va(Loads(k),1);
    VbT(k)=Vb(Loads(k),1);
    VcT(k)=Vc(Loads(k),1);
